...
const tutorialDetail = require('./content/sw-tutorial-detail');
...
data.parts.forEach((p, idx) => {
  ...
  const content = buildTutorialBody({
    seriesName: 'SolidWorks AI 自动化',
    page: p.page,
    shortTitle: titles[idx],
    themeName: `系列第 ${p.page}/6 步`,
...
3. **产出物**：${outputs[idx]}
4. **学习侧重**：${examTip}
5. **笔记层级**：教程级（约 ${wc} 字），含流水线图解、实操 Walkthrough、Checklist

> 以下内容基于 SolidWorks API 与机械设计知识体系撰写，对应 B 站分 P「${p.part}」。**非 UP 逐字转写**；不看视频可按 Walkthrough 纸面演练，看视频对照操作细节。
...
- ✅ **教程级增强**：含 Mermaid 流水线、实操 Walkthrough、自测题（约 ${wc} 字，${DATE}）
