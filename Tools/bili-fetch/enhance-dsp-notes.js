...
const tutorialDetail = require('./content/dsp-tutorial-detail');
...
data.parts.forEach((p, idx) => {
  ...
  const content = buildTutorialBody({
    seriesName: 'DSP 数字信号处理',
    page: p.page,
    shortTitle,
    themeName: `第 ${ch.num} 章 · ${ch.name}`,
    prevTitle,
    nextTitle,
...
up: "${data.up}"
tags: [dsp, 数字信号处理, 视频笔记, 西电, 第${ch.num}章, 教程级]
...
3. **考试侧重**：${examTip}
4. **笔记层级**：教程级（约 ${wc} 字），含速览、图解、例题 Walkthrough、自测题
5. **学习建议**：先读「3 分钟速览」，手算 1 题后再看视频核对步骤

> 以下内容基于西电版《数字信号处理》教材知识体系撰写，对应 B 站分 P「${p.part}」。**非 UP 逐字转写**；不看视频可建立框架，看视频对照「与视频对照表」。
...
- ✅ **教程级增强**：含 Mermaid、例题 Walkthrough、自测题（约 ${wc} 字，${DATE}）
