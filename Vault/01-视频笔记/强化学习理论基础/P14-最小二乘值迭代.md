---
title: "P14 最小二乘值迭代 (Least-Squares Value Iteration)"
source: "https://www.bilibili.com/video/BV1r6cjeCEkW?p=14"
up: "Proof-Trivial"
...
# P14 最小二乘值迭代 (Least-Squares Value Iteration)

← [[BV1r6cjeCEkW-总览]] | ← [[P13-大状态空间中的强化学习]] | 下一篇 → [[P15-大状态空间中的探索Space]]

## 视频信息

| 项目 | 内容 |
|------|------|
| 分集 | 最小二乘值迭代 (Least-Squares Value Iteration) |
| 模块 | 大状态空间与函数逼近 |
| 时长 | 1 小时 17 分 21 秒 |
| 链接 | [B 站 P14](https://www.bilibili.com/video/BV1r6cjeCEkW?p=14) |
| 课程主页 | [Chi Jin ECE524](https://sites.google.com/view/cjin/teaching/ece524) |
| 内容来源 | 知识点增强（RL 理论体系，非逐字转写） |

## 核心要点

1. **本 P 主题**：最小二乘值迭代 (Least-Squares Value Iteration)
2. **模块定位**：大状态空间与函数逼近（P13–P17）
3. **考试/实践侧重**：LSVI-UCB 算法、岭回归置信界、O(√d³H³K) regret
4. **笔记层级**：教程级（约 3475 字），含速览、图解、Walkthrough、自测题
5. **学习建议**：先通读「3 分钟速览」与「图解」，再读「详细讲解」

> 以下内容基于 Princeton ECE524 强化学习理论课程体系撰写，对应 B 站分 P「【14】最小二乘值迭代 (Least-Squares Value Iteration)」。**非 UP 逐字转写**；不看视频也可建立框架，看视频可对照「与视频对照表」深化。

## 本节在系列中的位置

**模块**：大状态空间与函数逼近（P13–P17）· 系列第 **P14/22** 集。
...
**最小二乘值迭代** 是 Princeton ECE524 强化学习理论核心一讲。读完本节你应能：① 复述核心定义与定理；② 说明在探索/逼近/博弈链条中的位置；③ 完成一道典型推导或算法步骤。考试/面试侧重：**LSVI-UCB 算法、岭回归置信界、O(√d³H³K) regret**。

## 零基础导读

本节「最小二乘值迭代」属于 **大状态空间与函数逼近**。Princeton **Chi Jin** 课程强调**可证明的样本复杂度与 regret**，而非仅算法启发式。即便未看视频，也应先建立「定义 → 算法/定理 → 证明 sketch → 与前后讲衔接」四层结构。
...
> 本集总时长约 **77分21秒**。无官方外挂字幕时，以分 P 标题「最小二乘值迭代 (Least-Squares Value Iteration)」与上表主题对齐视频画面。
...
- → **大状态空间中的探索 (Exploration in Large State) Space**（[[P15-大状态空间中的探索Space]]）

## 来源说明

- ✅ B 站官方元数据（`Tools/BV1r6cjeCEkW-full.json`）
- ✅ 分 P 首帧封面（`Tools/bili-fetch/fetch-bilibili.js`）
- ✅ **教程级增强**：含 Mermaid、Walkthrough、自测题（约 3475 字，2026-06-06）
- ⏳ 逐字转写：API 无外挂字幕轨；可选 Whisper/BiliNote 后续补充

## 关键截图

![[../../06-资源附件/video-notes-images/BV1r6cjeCEkW-P14-cover.jpg|B站首帧 P14]]
