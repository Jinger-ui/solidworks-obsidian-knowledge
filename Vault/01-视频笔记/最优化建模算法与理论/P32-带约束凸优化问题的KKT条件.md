---
title: "P32 带约束凸优化问题的KKT条件"
source: "https://www.bilibili.com/video/BV1Kc411i7kJ?p=32"
up: "Proof-Trivial"
tags: [最优化, 凸优化, 视频笔记, lagrange, 北大, 文再文, 教程级]
duration: "26m21s"
cid: 1354614578
created: 2026-06-06
updated: 2026-06-06
...
← [[BV1Kc411i7kJ-总览]] | ← [[P31-凸优化问题的Slater条件]] | 下一篇 → [[P33-最优化问题解的存在性]]

## 视频信息

| 项目 | 内容 |
|------|------|
| 分集 | 6.7 带约束凸优化问题的KKT条件 |
| 模块 | 第6章 拉格朗日对偶 |
| 时长 | 26 分 21 秒 |
| 链接 | [B 站 P32](https://www.bilibili.com/video/BV1Kc411i7kJ?p=32) |
| 教材 | 文再文《最优化：建模、算法与理论》 |
| 内容来源 | 知识点增强（教材体系，非逐字转写） |

## 核心要点

1. **本 P 主题**：带约束凸优化问题的KKT条件
2. **模块定位**：第6章 拉格朗日对偶（P26–P32）
3. **考试/实践侧重**：凸问题 KKT：∇f+∑λ_i∇g_i+∑ν_j∇h_j=0、互补松弛
4. **笔记层级**：教程级（约 3053 字），含速览、图解、Walkthrough、自测题
5. **学习建议**：先通读「3 分钟速览」与「图解」，再读「详细讲解」

> 以下内容基于北大文再文最优化课程体系撰写，对应 B 站分 P「6.7 带约束凸优化问题的KKT条件」。**非 UP 逐字转写**；不看视频也可建立框架，看视频可对照「与视频对照表」深化。

## 本节在系列中的位置

**模块**：第6章 拉格朗日对偶（P26–P32）· 系列第 **P32/101** 集。
...
**带约束凸优化问题的KKT条件** 是北大文再文最优化课程核心一讲。读完本节你应能：① 复述核心定义与定理；② 说明在 16 章知识体系中的位置；③ 完成一道典型推导或算法步骤。考试/面试侧重：**凸问题 KKT：∇f+∑λ_i∇g_i+∑ν_j∇h_j=0、互补松弛**。

## 零基础导读

本节「带约束凸优化问题的KKT条件」属于 **第6章 拉格朗日对偶**。文再文课程强调**建模—理论—算法**三位一体，与教材《最优化：建模、算法与理论》同步。即便未看视频，也应先建立「定义 → 定理/算法 → 收敛分析 → 与前后讲衔接」四层结构。

第一遍盯住：本讲**解决什么问题**？**关键假设**（凸性、光滑性、Lipschitz、CQ）是什么？**算法复杂度/收敛率**如何表述？第二遍对照教材例题补全证明细节。

## 详细讲解

### 1. 本讲主题：带约束凸优化问题的KKT条件

本讲属于**文再文《最优化：建模、算法与理论》** 第6章 拉格朗日对偶（P26–P32），对应 B 站分 P **6.7 带约束凸优化问题的KKT条件**。

**考试/面试侧重**：凸问题 KKT：∇f+∑λ_i∇g_i+∑ν_j∇h_j=0、互补松弛。

### 2. 核心定义与定理

在 第6章 拉格朗日对偶 框架下，「带约束凸优化问题的KKT条件」建立以下主干：
...
- **前置**：「凸优化问题的Slater条件」提供本讲所需概念。
- **后续**：「最优化问题解的存在性」将在本讲基础上深入。

### 8. 复习要点

1. 闭卷写出 第6章 拉格朗日对偶 与本讲相关的 2–3 个定义或定理。
2. 独立完成一道与「带约束凸优化问题的KKT条件」相关的计算或证明 sketch。
3. 对照教材 第6章 拉格朗日对偶 习题，标注易错点。
...
KKT 像**平衡木上的静止**：梯度被约束「推」到零，互补松弛说「不紧的约束不产生力」。
...
| 文再文 | 北京大学教授，教材作者 |
| 本讲关键词 | 带约束凸优化问题的KKT条件 |

## 与前后分 P 的衔接

- ← **凸优化问题的Slater条件**（[[P31-凸优化问题的Slater条件]]）
- → **最优化问题解的存在性**（[[P33-最优化问题解的存在性]]）

## 来源说明

- ✅ B 站官方元数据（`Tools/BV1Kc411i7kJ-full.json`）
- ✅ 分 P 首帧封面（`Tools/bili-fetch/fetch-bilibili.js`）
- ✅ **教程级增强**：含 Mermaid、Walkthrough、自测题（约 3053 字，2026-06-06）
- ⏳ 逐字转写：API 无外挂字幕轨；可选 Whisper/BiliNote 后续补充

## 关键截图

![[../../06-资源附件/video-notes-images/BV1Kc411i7kJ-P32-cover.jpg|B站首帧 P32]]
