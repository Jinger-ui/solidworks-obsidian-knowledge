---
title: "P65 有限内存BFG1方法 (2)"
source: "https://www.bilibili.com/video/BV1Kc411i7kJ?p=65"
up: "Proof-Trivial"
...
← [[BV1Kc411i7kJ-总览]] | ← [[P64-有限内存BFGS方法]] | 下一篇 → [[P66-非线性最小二乘问题]]

## 视频信息

| 项目 | 内容 |
|------|------|
| 分集 | 11.5 有限内存BFG1方法 (2) |
| 模块 | 第11章 拟牛顿法 |
| 时长 | 14 分 20 秒 |
| 链接 | [B 站 P65](https://www.bilibili.com/video/BV1Kc411i7kJ?p=65) |
| 教材 | 文再文《最优化：建模、算法与理论》 |
| 内容来源 | 知识点增强（教材体系，非逐字转写） |

## 核心要点

1. **本 P 主题**：有限内存BFG1方法 (2)
2. **模块定位**：第11章 拟牛顿法（P61–P66）
3. **考试/实践侧重**：L-BFGS 大规模 ML 训练、批处理
4. **笔记层级**：教程级（约 3097 字），含速览、图解、Walkthrough、自测题
5. **学习建议**：先通读「3 分钟速览」与「图解」，再读「详细讲解」

> 以下内容基于北大文再文最优化课程体系撰写，对应 B 站分 P「11.5 有限内存BFG1方法 (2)」。**非 UP 逐字转写**；不看视频也可建立框架，看视频可对照「与视频对照表」深化。

## 本节在系列中的位置

**模块**：第11章 拟牛顿法（P61–P66）· 系列第 **P65/101** 集。
...
**有限内存BFG1方法 (2)** 是北大文再文最优化课程核心一讲。读完本节你应能：① 复述核心定义与定理；② 说明在 16 章知识体系中的位置；③ 完成一道典型推导或算法步骤。考试/面试侧重：**L-BFGS 大规模 ML 训练、批处理**。

## 零基础导读

本节「有限内存BFG1方法 (2)」属于 **第11章 拟牛顿法**。文再文课程强调**建模—理论—算法**三位一体，与教材《最优化：建模、算法与理论》同步。即便未看视频，也应先建立「定义 → 定理/算法 → 收敛分析 → 与前后讲衔接」四层结构。
...
- ← **有限内存BFGS方法 (1)**（[[P64-有限内存BFGS方法]]）
- → **非线性最小二乘问题**（[[P66-非线性最小二乘问题]]）

## 来源说明

- ✅ B 站官方元数据（`Tools/BV1Kc411i7kJ-full.json`）
- ✅ 分 P 首帧封面（`Tools/bili-fetch/fetch-bilibili.js`）
- ✅ **教程级增强**：含 Mermaid、Walkthrough、自测题（约 3097 字，2026-06-06）
- ⏳ 逐字转写：API 无外挂字幕轨；可选 Whisper/BiliNote 后续补充

## 关键截图

![[../../06-资源附件/video-notes-images/BV1Kc411i7kJ-P65-cover.jpg|B站首帧 P65]]
